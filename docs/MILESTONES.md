...
| D2 | Enrollment signing mode **(a)**: `uploadd` signs CSRs directly. Mode (b) documented as an option in the server spec. **Superseded in part by D8** — the "temporarily place `ca.key`, remove it after" ceremony is no longer the default. | FRD recommendation. With D1, enrollment happens once per device. The ceremony turned out to have a real UX cost after all: it is per-enrollment, not one-off, and it blocks a clean open-source quickstart. |
...
| D8 | **`docker compose up` is the entire server setup** (added post-M7, for open-sourcing). `uploadd` provisions its own CA, server certificate and empty CRL on first run — `bootstrap.sh` and `sign-csr.sh` are deleted and `openssl` is no longer a host dependency. **This amends D2 and §1.2 rule 5: `ca.key` now stays on the server by default.** The tradeoff is deliberate and stated in the README — read access to `ca.key` is enough to mint a device certificate — but the alternative was a file-shuffling ritual on every enrollment, which is the single biggest barrier to anyone else running this. Trust still reaches the phone out of band: the pairing QR carries the CA *and* its fingerprint, and the app rejects a CA whose fingerprint does not match, so no file is ever copied by hand. The offline-key posture remains supported (move `ca.key` away; enrollment answers `503` until it returns). |
