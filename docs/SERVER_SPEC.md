...
5. `ca.key` is not stored in any running container's writable volume and is not
...
server/
  ...
  ca/
    bootstrap.sh            # one-command CA + server cert (run once)
    sign-csr.sh             # offline signing helper (enrollment mode b)
    revoke.sh               # revoke a device cert + regenerate CRL
  admin/
...
All key material is EC P-256. Tooling is plain `openssl` — no CA framework.
...
```
./bootstrap.sh <hostname> [lan-ip]
```

- Runs exactly once; **refuses to overwrite** an existing CA (exit non-zero with
  a clear message).
- Produces `ca.key`, `ca.crt`, `server.key`, `server.crt`, `crl.pem` (empty
  initial CRL), and `ca.srl`/index files for signing.
- On completion prints an **unmissable warning**: move `ca.key` (and the CA
  index/serial state needed for signing) to offline storage and delete it from
  this host. Server operation needs only `ca.crt`, `server.crt`, `server.key`,
  `crl.pem`.

### 3.3 `ca/sign-csr.sh`

`./sign-csr.sh <csr-file> <device-label>` — signs a PKCS#10 CSR with the CA key,
25-year validity, EKU `clientAuth`, CN forced to `<device-label>`. Used by
`uploadd` during enrollment (mode a) and manually (mode b).

### 3.4 `ca/revoke.sh`

`./revoke.sh <serial>` — marks the cert revoked in the CA index, regenerates
`crl.pem`, and prints the command to reload nginx
(`docker compose exec nginx nginx -s reload`). Requires `ca.key` temporarily
restored, same as enrollment.

### 3.5 Enrollment signing mode (decision D2)

Default is mode **(a)**: the admin page instructs the user to temporarily place
`ca.key` in the CA directory before enrolling, and remove it afterward.
`uploadd` invokes signing directly. Mode **(b)** — CSR parked in
`./state/pending/`, signed offline with `sign-csr.sh`, app polls for the result —
must be documented in the README as the paranoid option, but is not required to
be implemented in v1 of the backend.
...
Called by the admin page (button click), not the app. Creates a pairing token
(single use, **TTL 10 minutes**) and returns the QR payload:
